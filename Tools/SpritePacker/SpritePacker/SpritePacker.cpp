    ...
    m_NeedToSort(false)
{
    ...
    m_TopLefts.push_back(Coordinate(0, 0));
...
bool SpritePacker::DoesFit(const Bin& aBin) const
{
    ...
    if ((aBin.coordinate.x + aBin.size.width) > m_AtlasWidth)
    {
        return false;
    }
        
    //Check to see if the bin fits vertically
    if ((aBin.coordinate.y + aBin.size.height) > m_AtlasHeight)
    {
        return false;
    }
...
void SpritePacker::Use(const Bin& aBin) 
{
    ...
    m_TopLefts.push_front(Coordinate(coordinate.x + size.width, coordinate.y));
    m_TopLefts.push_back(Coordinate(coordinate.x, coordinate.y + size.height));
...
void SpritePacker::Trim()
{
    ...
    m_AtlasWidth = boundsRight;
    m_AtlasHeight = boundsBottom;
}