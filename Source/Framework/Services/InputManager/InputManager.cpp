...
namespace GameDev2D
{
	...
	void InputManager::RegisterLeftMouseClickCallback(std::function<void(float, float)> aLeftMouseClickCallback)
	{
		m_LeftMouseClickCallback = aLeftMouseClickCallback;
	}

	void InputManager::RegisterRightMouseClickCallback(std::function<void(float, float)> aRightMouseClickCallback)
	{
		m_RightMouseClickCallback = aRightMouseClickCallback;
	}

	void InputManager::RegisterKeyPressedCallback(std::function<void(Keyboard::Key)> aKeyPressedCallback)
	{
    ...
    void InputManager::HandleLeftMouseUp(float aX, float aY)
    {
		...
		//Handle the callback
		if (m_LeftMouseClickCallback != nullptr)
		{
			m_LeftMouseClickCallback(m_Mouse->GetPosition().x, m_Mouse->GetPosition().y);
		}
    }
    ...
    void InputManager::HandleRightMouseUp(float aX, float aY)
    {
		...
		//Handle the callback
		if (m_RightMouseClickCallback != nullptr)
		{
			m_RightMouseClickCallback(m_Mouse->GetPosition().x, m_Mouse->GetPosition().y);
		}
    }
