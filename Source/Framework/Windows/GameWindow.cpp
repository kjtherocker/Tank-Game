...
namespace GameDev2D
{
    ...
    void GameWindow::HandleResize(unsigned int aWidth, unsigned int aHeight)
    {
        ...
        if (aWidth != m_Width || aHeight != m_Height)
        {
            ...
            if (Services::GetGraphics() != nullptr && Services::GetGraphics()->GetCamera())
            {
                Services::GetGraphics()->GetCamera()->SetViewport(Viewport(m_Width, m_Height));
            }
    ...
    LRESULT GameWindow::EventHandler(HWND aHandle, UINT aMessage, WPARAM aWParam, LPARAM aLParam)
    {
	    ...
	    switch (aMessage)
	    {
	        ...
	        case WM_SYSKEYUP:
            {
                Services::GetInputManager()->HandleKeyUp(MapKey(aWParam, aLParam));
            }
	        break;
