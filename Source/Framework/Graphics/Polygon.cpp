...
namespace GameDev2D
{ 
    Polygon* Polygon::CreateCircle(float aRadius, unsigned int aLineSegments, bool aIsFilled)
    {
        //The radius can't be negative OR zero
        assert(aRadius > 0.0f);

        //Create the Polygon and set it's render mode
        Polygon* polygon = new Polygon();
        polygon->SetRenderMode(aIsFilled == true ? RenderMode_TriangleFan : RenderMode_LineLoop);
    ...
    Polygon* Polygon::CreateRectangle(float aWidth, float aHeight, bool aIsFilled)
    {
        ...
        //Add the vertices
        polygon->AddVertex(Vector2(0.0f, 0.0f));
        polygon->AddVertex(Vector2(aWidth, 0.0f));
        polygon->AddVertex(Vector2(aWidth, aHeight));
        polygon->AddVertex(Vector2(0.0f, aHeight));
    ...
    Polygon::Polygon() : Drawable(),
        ...
        m_RenderMode(RenderMode_LineLoop),
    ...
    void Polygon::SetBlendingMode(BlendingFactor aSource, BlendingFactor aDestination)
    {
        m_BlendingMode.source = aSource;
        m_BlendingMode.destination = aDestination;
    }

    void Polygon::SetBlendingMode(BlendingMode aBlendingMode)
    {
        m_BlendingMode = aBlendingMode;
    }

    BlendingMode Polygon::GetBlendingMode()
    {
