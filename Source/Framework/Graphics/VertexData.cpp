...
namespace GameDev2D
{
    ...
    void VertexBuffer::UpdateBuffer()
    {
        ...
        if (GetCount() == 0)
        {
            Log::Error(this, "VertexBuffer", false, Log::Verbosity_Graphics, "UpdateBuffer() failed, there are no vertices");
            return;
        }
    ...
    void IndexBuffer::UpdateBuffer()
    {
        ...
        if (GetCount() == 0)
        {
            Log::Error(this, "IndexBuffer", false, Log::Verbosity_Graphics, "UpdateBuffer() failed, there are no indices");
            return;
        }
