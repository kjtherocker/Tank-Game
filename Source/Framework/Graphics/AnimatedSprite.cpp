...
namespace GameDev2D
{
    AnimatedSprite::AnimatedSprite(const std::string& aFilename) : SpriteAtlas(aFilename), EventHandler(),
        m_FrameIndex(0),
        m_FrameSpeed(15.0f),
        m_ElapsedTime(0.0),
        m_DoesLoop(false)
    ...
    void AnimatedSprite::HandleEvent(Event* aEvent)
    {
        ...
        if (aEvent->GetEventCode() == UPDATE_EVENT)
        {
            ...
            Log::Message("%i, %f, %f", frameIndex, delta, Services::GetApplication()->GetGameLoop()->GetElapsedTime());
